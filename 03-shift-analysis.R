...
(bcl_pvals < .05 / 5) %>% which %>% names # only Bcl2l13:del/del
...
test_shift <- function(gt){
  shift <- filter(df, Genotype_mother == gt)$Shift
...
test_df <- gt_nms %>% 
  lapply(test_shift) %>% 
  bind_rows()
test_df$Genotype_mother <- factor(test_df$Genotype_mother, levels = gt_nms)
...
median_pvals <- split(test_df$pval, test_df$Genotype_mother) %>% 
  ...
  round(3)
median_pvals
(median_pvals < psig) %>% which %>% names # "Bcl2l13:del/del" "Ulk1:del/del" 
